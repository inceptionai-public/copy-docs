...
Contact Support to complete this step.
...
* Your **CMK ARN** from step 1 (for example, `arn:aws:kms:<region>:account-id>:key/<key-id>`).
* Your **Role ARN** from step 2 (for example, `arn:aws:iam::<account-id>:role/<role-name>`).
