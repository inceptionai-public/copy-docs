# Amazon Q Integration with (In)Business Productivity Platform

In addition to Search and Data AI solution, (In)Business Productivity Platform connects with external search indexes such as Amazon Q. This integration allows customers to leverage connectors configured in their Amazon Q Index application on top of the ones available.
...
This final step completes the integration by configuring Inception as a data accessor using the Tenant ID provided during the Platform setup.
...
2. **Select the data accessor**:
    * Choose the appropriate data accessor from the available options.
