...
!!!note
    In the Platform, the encryption key configuration is implemented at the account level, unlike in the (In)Business Customer Experience Platform, where it's applied at the app level. As a result, all apps within a given account in the Platform use the same encryption key. 
...
3. Select the Cloud provider and provide the configuration details. The Platform supports 
